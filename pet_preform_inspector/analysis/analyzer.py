...
from measurement.measuring_size import measure_gabarits, measure_bottom_diameter
...
class PreformGeometryAnalyzer:
    ...
    def analyze(self, img: np.ndarray, image_id: str = "unknown") -> Tuple[str, Dict, List]:
        try:
            ...
            # Явная проверка: если контур не прошёл фильтрацию — отбраковка
            if approx is None:
                self.logger.warning(
                ...
                # --- ВИЗУАЛИЗАЦИЯ ОТБРАКОВКИ (ТОЛЬКО OPEN CV, АНГЛИЙСКИЙ) ---
            ...
            # --- ШАГ 2: Выравнивание контура по главной оси ---
            ...
            # --- ШАГ 4: Измерение габаритных размеров (L, Z) ---
            gabarit_data_before = measure_gabarits(
                aligned_approx,
                pixels_per_mm=self.pixels_per_mm
            )
            extreme_before = gabarit_data_before["extreme_points"]

            # --- ШАГ 5: ОПРЕДЕЛЕНИЕ ОРИЕНТАЦИИ (без переворота) ---
            ...
            # --- ШАГ 7: Подготовка плотного выровненного контура для измерения дна ---
            ...
            # --- ШАГ 8: Измерение диаметра дна (d1) с учётом ориентации ---
            bottom_data = None
            d1_px = 0.0
            d1_mm = 0.0
            ...
            if config.STEP_3_MEASURE_DIMENSIONS and rot_mat is not None:
            ...
            # --- ВИЗУАЛИЗАЦИЯ ОШИБКИ (АНГЛИЙСКИЙ ТЕКСТ!) ---
            error_img = img.copy()
